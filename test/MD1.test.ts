...
import { MerkleDistributor1__factory, MockERC20__factory } from "../typechain";
import { getRandomInt, get_MerkleTree } from "../util/util";
describe("Merkle distirubtor version 1 test", () => {
  ...
  it("Should transfer airdrop token to target account", async () => {
    ...
    await merkleDistributor.claim(testAddr, mockERC20.address, merkleTree?.getHexProof(leafNode));
  ...
  it("Should use right MerkleProof", async () => {
    ...
    while (accountNum === accountNum1)
        accountNum1 = getRandomInt(1, 100);
    const testAddr1 = accountList[accountNum1];
    const leafNode1 = ethers.utils.keccak256(Buffer.from(testAddr1.replace("0x", ""), "hex"));
    await expect(merkleDistributor.claim(testAddr, mockERC20.address, merkleTree?.getHexProof(leafNode1))).revertedWith("Invalid Proof.");
  ...
  it("Should not claim double or more times", async function () {
    ...
    await mockERC20.transfer(merkleDistributor.address, ethers.utils.parseEther("10000"));
    await merkleDistributor.claim(testAddr, mockERC20.address, merkleTree?.getHexProof(leafNode));
    await expect(merkleDistributor.claim(testAddr, mockERC20.address, merkleTree?.getHexProof(leafNode))).revertedWith("Already claimed.");
  });
  ...
  it("Should have enough airdrop tokens in contract", async function () {
    ...
    await expect(merkleDistributor.claim(testAddr, mockERC20.address, merkleTree?.getHexProof(leafNode))).revertedWith("ERC20: transfer amount exceeds balance");
  });

  it("Should use correct airdrop token address", async function () {
    const { mockERC20, merkleDistributor, merkleTree, testAddr, leafNode } = await loadFixture(deployFixture);
    await mockERC20.transfer(merkleDistributor.address, ethers.utils.parseEther("10000"));
    await expect(merkleDistributor.claim(testAddr, testAddr, merkleTree?.getHexProof(leafNode))).revertedWith("Transaction reverted: function returned an unexpected amount of data");
  });
});
